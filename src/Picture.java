...
public class Picture extends SimplePicture {
	...
	public void grayscale() {
		...
		for (Pixel[] rowArray : pixels) {
			for (Pixel pixelObj : rowArray) {
				int avg = pixelObj.getRed() + pixelObj.getBlue() + pixelObj.getGreen();
				avg /= 3;
				pixelObj.setRed(avg);
	...
	public void mirrorArms() {
		...
		for (int row = 150; row < 200; row++) {
			for (int col = 100; col < 170; col++) {
				topPixel = pixels[row][col];
				botPixel = pixels[200-row+200][col];
				botPixel.setColor(topPixel.getColor());
			}	
		}
	...
	public void copy(Picture fromPic, int startrow, int startcol, int endrow, int endcol, int copyToRow, int copyToCol) {
		...
		Pixel[][] toPixels = this.getPixels2D();
		Pixel[][] fromPixels = fromPic.getPixels2D();
		for (int fromRow = startrow, toRow = copyToRow; 
			fromRow <= endrow && toRow < toPixels.length; 
			fromRow++, toRow++) {
			for (int fromCol = startrow, toCol = copyToRow; 
